...
class DevSecOpsApp {
  ...
  // Load configuration from global (injected) window.APP_CONFIG
  ...
  updateTraceIdDisplay() {
    ...
    displays.forEach(id => {
      const element = document.getElementById(id);
      if (element) element.textContent = this.traceId;
    });
  }
  ...
  displayHealthStatus(healthData) {
    ...
    const iconClass = isHealthy ? 'fas fa-check-circle' : 'fas fa-exclamation-triangle';
  ...
  displayHealthError(errorMessage) {
    ...
    container.innerHTML = `
      <div class="alert alert-danger">
        <h5><i class="fas fa-exclamation-triangle"></i> Unable to fetch system health status</h5>
        ...
        <small>Please check if backend service is running on port ${backendPort}</small>
      </div>
    `;
  }
  ...
  displayUsers(users) {
    ...
    tbody.innerHTML = users.map(user => `
      <tr>
        ...
        <td>
          <button class="btn btn-sm btn-outline-primary me-1" onclick="app.viewUser(${user.id})">View</button>
          <button class="btn btn-sm btn-outline-warning me-1" onclick="app.editUser(${user.id})">Edit</button>
          <button class="btn btn-sm btn-outline-danger" onclick="app.deleteUser(${user.id})">Delete</button>
        </td>
      </tr>
    `).join('');
  }
      ...
      <tr>
        <td colspan="6" class="text-center">
          <div class="alert alert-danger mb-0">
            ...
            <small>Backend service may be unavailable on port ${backendPort}</small>
          </div>
        </td>
      </tr>
    `;
  ...
  async deleteUser(userId) {
    ...
    try {
      await this.makeRequest(`${this.baseURL}/api/users/${userId}`, { method: 'DELETE' });
